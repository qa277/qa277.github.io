const game = document.getElementById("game");
    ...
    serverRequest.open("GET", "https://qa277.github.io/" + songSwitcher.list[songSwitcher.index] + ".json");
    serverRequest.send();
    ...
    document.body.onkeyup = function(e){
      if(e.keyCode == 37){
        if (songSwitcher.index > 0) {
          ...
          q.open("GET", "https://qa277.github.io/" + songSwitcher.list[songSwitcher.index] + ".json");
          q.send();
          q.onload = function() {
  ...
  function draw() {
    ...
    let textSize = (game.width * 0.5)/main.content.length
 	  ...
 	  display.font = textSize + "px Courier";
    ...
    if (hover({x: game.width / 2 + 350, y: game.height / 2 - 40, width: 100,height: 100})) {
 	...
 	display.font = "20px Courier";
 	display.textAlign = "left"
 	if (localStorage[sn] === undefined) {
 		console.log("It\'s null!")
 		text("No highscore found :(", 30, 40, main.textColor, game.width);
 	} else {
