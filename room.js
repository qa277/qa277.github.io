...
function randomText() {
    ...
    for (let i = 0; i < 7; i++) {
        result += chars[randomNumber(0, chars.length - 1)];
    }
    return result;
}
...
function all() {
    ...
    if (serverData == null) {
        ...
        if (serverKeys != null) {
            for (let i = 0; i < serverKeys.length; i++) {
                if (serverKeys[i].playerCount == 1 && serverKeys[i].turn == 0) {
...
document.getElementById("create-room").onclick = function() {
    ...
    if (result == true) {
        ...
        document.getElementById("a").appendChild(canvas)
        document.getElementById("rename").innerHTML = "";
        document.getElementById("room-id-display").innerHTML = "Room Id: " + roomId;
    }
}
...
function getData(data) {
    ...
    if (serverData == null) {
        ...
        if (displayPubs && roomDeployed != true) {
            document.querySelectorAll('.room').forEach(e => e.remove());
            all();
        }
    }
    for (let i = 0; i < serverKeys.length; i++) {
