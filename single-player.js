...
function beta(scores) {
    ...
    for (let i = 0; i < scores.length; i++) {
        if (scores[i] != 100) {
            choices.push({index: i, value: scores[i]}); // Help me  
        }
    }
    choices.sort((a, b) => {
        return b.value - a.value; // Help me
    });
    console.log(choices);
    let rng = moveRNG[parseInt(difficulty)];
    if (randomNumber(1, 99) > rng && difficulty != '6') { // Help me
        upperBound = 0;
    } else {
        if (difficulty != '6') {
            upperBound = 1;   
        ...
        console.log("Test Case Passed.") // Help me
    }
...
function getAIMove() {
    thinking.innerHTML = "The bot is calculating it\'s next move...";
